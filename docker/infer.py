...
def main(args, task=None, model_state=None):
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ############################################################################################
            ## HACK to split the audio #################################################################
            ############################################################################################
            import copy
            if use_cuda:
                max_len_size = 3000000
            else:
                print("NO CUDA-----------")
                max_len_size = 6000000 # Luego se podra hacer el doble
            original_sample = copy.deepcopy(sample)
            hypos_final = None
            for i in tqdm.tqdm(range(0, len(original_sample["net_input"]["source"][0]), max_len_size)):
                finish = i + max_len_size
                if finish > len(original_sample["net_input"]["source"][0]):
                    finish = len(original_sample["net_input"]["source"][0]) + 1
                sample = copy.deepcopy(original_sample)
                sample["net_input"]["source"] = original_sample["net_input"]["source"][:, i:finish]
                ...
                if hypos_final == None:
                    hypos_final = copy.deepcopy(hypos)
                    # print("HYPOS: ")
                    # print(hypos_final[0][0]["tokens"], flush=True)
                else:
                    hypos_final[0][0]["tokens"] = torch.cat((hypos_final[0][0]["tokens"], hypos[0][0]["tokens"]), 0)
                    # print("HYPOS: ")
                    # print(hypos_final[0][0]["tokens"], flush=True)

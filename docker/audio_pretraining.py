...
class AudioPretrainingConfig(FairseqDataclass):
    ...
    max_sample_size: Optional[int] = field(
        default=320000, metadata={"help": "max sample size to crop to for batching"}
    )
    min_sample_size: Optional[int] = field(
        default=32000, metadata={"help": "min sample size to skip small examples"}
    )

    # Options for reporting WER metrics during validation. Only applicable to
    # Seq2Seq models during fine-tuning
    eval_wer: bool = field(
        default=True, metadata={"help": "compute WER for Seq2Seq models"}
    )
    ...
    conv_feature_layers: str = field(
        default='[(512, 10, 5)] + [(512, 3, 2)] * 4 + [(512,2,2)] + [(512,2,2)]', metadata={"help": ""}
    )
    encoder_embed_dim: int = field(
        default=1024, metadata={"help": ""}
    )
